-- =============================================================================
-- My Kenyan Guide — Enhance Listings Table
-- Adds County → Constituency → Area Code/Location hierarchy and
-- Category 2 (Businesses That Offer Services) specific fields.
...
alter table public.listings
  add column if not exists constituency text not null default '',
  add column if not exists area_code text not null default '',
  ...
  add column if not exists charges text not null default '';

-- ---------------------------------------------------------------------------
-- 2. Update search_vector function to include new columns
-- ---------------------------------------------------------------------------
...
create trigger trg_listings_search_vector
  before insert or update of title, subtitle, description, location, tags, constituency, area_code, keywords, service_type
  on public.listings
  for each row execute function public.update_listing_search_vector();

-- ---------------------------------------------------------------------------
-- 3. New indexes for the added columns
-- ---------------------------------------------------------------------------
create index if not exists idx_listings_constituency on public.listings (constituency);
create index if not exists idx_listings_area_code on public.listings (area_code);
create index if not exists idx_listings_keywords on public.listings using gin (keywords);
create index if not exists idx_listings_service_type on public.listings (service_type);

-- ---------------------------------------------------------------------------
-- 4. Update the search_listings RPC to support fuzzy/typo-tolerant search
--    using pg_trgm similarity alongside full-text search
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 5. Update the get_or_create_conversation function to use listing title
-- ---------------------------------------------------------------------------
-- (no changes needed — existing function still works)